...
set up an enture cluster, including DRS, vSAN, HA, vDS, ESX networking
...
def GetFreePnicList(host):
   ...
   # link-state is UP.
   allUpPnics = map(lambda z: z.device,
   ...
   # connected to each.
   usedNicsOnVss = map(lambda z: z.spec.bridge.nicDevice,
   ...
   # pnic name.
   usedNicsOnProxy = map(lambda y: y[0],
      map(lambda z: z.pnic,
...
def main():
   ...
   clusterSpec = vim.ClusterConfigSpecEx(inHciWorkflow = True)
   ...
   apiVersion = vsanapiutils.GetLatestVmodlVersion(args.vc)
